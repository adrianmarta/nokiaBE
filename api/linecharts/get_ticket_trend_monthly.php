...
function getMonthLabel(DateTime $date) {
...
foreach ($tickets as $ticket) {
    if (!empty($ticket['start_date'])) {
        ...
        $result[$monthLabel]['startCount']++;
        $result[$monthLabel]['startedTickets'][] = $ticket;
    }
    if (!empty($ticket['closed_date'])) {
        $closed = $ticket['closed_date'];
        if (!$closed instanceof DateTime) {
            $closed = new DateTime($closed);
        }
        $monthLabel = getMonthLabel($closed);
        ...
        $result[$monthLabel]['closedCount']++;
        $result[$monthLabel]['closedTickets'][] = $ticket;
    }
}
